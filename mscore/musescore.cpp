...
void MuseScore::showRadar(bool visible)
      {
      QAction* a = getAction("radar");
      if (visible) {
            if (!radar) {
                  radar = new Radar();
     //             connect(radar, SIGNAL(radarVisible(bool)), a, SLOT(setChecked(bool)));
                  addDockWidget(Qt::RightDockWidgetArea, radar);
                  }
            if (cs) {
...
void MuseScore::ambiSendLocation(int channel, double r, double phi, double psi) {
      // Distanz ist Radius
      double d = r;
      
      // Elevation hängt nur von radius und psi ab
      double e = r*sin(psi);
      
      //phi auf [0, 2pi] normieren
    while(phi < 0) {
        phi += 2*M_PI;
    }
      while(phi > 2*M_PI) {
      ...
      double a = -phi*360.0/(2*M_PI) + 90.0;
      if(a< -180.) {
            a += 360.;
      }
      
      qDebug() << "ambiSend r:"<< r << " phi:" << phi << " psi:" << psi << "  =>  a:" << a << " e:" << e;
      
      // Skalieren auf Ambi-tools-normierung
      e *= 60;
      
      // Die Koordinaten für den Send vorbereiten
      QList<QVariant> args;
      args.append(QVariant::QVariant(channel));
      args.append(QVariant::QVariant(a));
      args.append(QVariant::QVariant(e));
      args.append(QVariant::QVariant(d));
      args.append(QVariant::QVariant(channel));
...
void MuseScore::oscSendMessage(const QString &m) {
      PathObject po(m, QVariant::Invalid, _oscSender);
      po.send();
}

void MuseScore::initAmbiOsc() {
      QOscClient* c = new QOscClient( QHostAddress::LocalHost, 5013, qApp );
      
      QList<QVariant> args;
      args.append(QVariant::QVariant(2));
      args.append(QVariant::QVariant(-90.0));
      args.append(QVariant::QVariant(40.0));
      args.append(QVariant::QVariant(.7));
      args.append(QVariant::QVariant(2));
      
      _ambiAed = new PathObject( "aed", QVariant::List, c );
      
      
      _oscSender = new QOscClient( QHostAddress::LocalHost, 5012, qApp );
}
