...
void Seq::start()
      ...
      calculateSpatDurations();
      }
      

void Seq::calculateSpatDurations() {
      
      // Alle SpatSymbole finden, so dass diese während dem Abspielen 
      // in Seq::hearbeat() aufgerufen werden können
      spatSymbols.clear();
      
      // Um die Duration zu bestimmen, müssen wir für jeden track (stimme) noch das letzte Symbol merken.
      QList<SpatSymbol*> lastSymbol;
      // HACK!!!
          for (int i=0; i<1000; i++) {
              lastSymbol.append(NULL);
          }
    ...
    QList<SpatSymbol*> trajectories;
    
      for (Segment* s = cs->firstSegment(); s; s = s->next1()) {
      
      		QList<SpatSymbol*> modifiers;
      
            foreach(Element* e, s->annotations()) {
                  // Nur die Spat-Symbole interessieren uns
                  if(!e || e->type() != Element::SPATIALIZATION_SYMBOL) {
                        continue;
                  }
                  SpatSymbol *spat = (SpatSymbol*) e;
                  
                  spat->resetSymbol();
                  
                  // Alle spatModifiers werden erst behandelt, nachdem wir alle Elemente des Segments behandelt habe
                  // So können Probleme vermieden werden, falls intern der modifier vor dem symbol steht
                  if(spat->spatFamily() == SPATFAMILY_MODIFIER) {
                  	modifiers.append(spat);
                  	continue;
                  }
                  
                  trajectories.append(spat);
                  
                  spat->_modifiers = QList<SpatSymbol*>();
                  spat->_beingRendered = true; 
                  spat->setDuration(0);
                  spatSymbols.append(spat);
                  
                  // Wir müssen noch die Länge des letzten Symbols auf dem track setzen
                  if(lastSymbol.at(spat->track()/VOICES) != NULL) {
                  	lastSymbol[spat->track()/VOICES]->setDuration(spat->tick() - lastSymbol[spat->track()/VOICES]->tick());
                  }
                  lastSymbol[spat->track()/VOICES] = spat;
                  
            }
            
            // Jetzt werden noch die modifiers zugeordnet
            foreach(SpatSymbol *spat, modifiers) {
            	if(lastSymbol.at(spat->track()/VOICES) != NULL) {
            		SpatSymbol *p = lastSymbol[spat->track()/VOICES];
            		spat->_modifier_parent = p;
                  	p->_modifiers.append(spat);
                  }
            }
      }
      
      // Die durations der Modifiers muss noch bestimmt werden
      foreach(SpatSymbol *t, trajectories) {
          for(int i=0; i<t->_modifiers.size(); i++) {
      		SpatSymbol *m = t->_modifiers.at(i);
      		if(i + 1 == t->_modifiers.size()) {
      			// Last Element
      			m->setDuration(t->tick() + t->duration() - m->tick());
      		}else{
      			m->setDuration( t->_modifiers.at(i+1)->tick() - m->tick());
      		}
      	}
      }
...
void Seq::putEvent(const Event& event, unsigned framePos)
      {
      if (!cs)
            return;
...
void Seq::heartBeat()
      ...
      // alle SpatSymbole über die aktuelle Position benachrichtigen
      foreach(SpatSymbol *s, spatSymbols) {
            SpatCoordinates c = s->calculateCoordinates(getCurTick()); 
            // TODO: Falls mehrere SpatSymbole eine Positionsmeldung für den gleichen Kanal haben,
            // so  darf nur die Richtige ausgeführt werden. 
            // Idee: Sortieren nach position und die Höchste nehmen.
            if(!c.ignore) {
                  mscore->ambiSendLocation(c.channel, c.r, c.phi, c.psi);
            }
            
            SpatOSCMessage m = s->oscMessage(getCurTick());
            if(!m.ignore) {
                  mscore->oscSendMessage(m.oscMessage());
            }
      }
