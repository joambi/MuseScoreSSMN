...
void ScoreView::createElementPropertyMenu(Element* e, QMenu* popup)
      ...
      else if (e->type() == Element::SPATIALIZATION_SYMBOL) {
            SpatSymbol* spat = static_cast<SpatSymbol*>(e);
            genPropertyMenu1(e, popup);
            QAction* a = popup->addSeparator();
            //a->setText(tr("Spatialization"));
            popup->addAction(tr("Select affected notes"))->setData("spatialization-select-affected-notes");
            }
...
//            s->setSubtype(TEXT_FRAME);
      ...
      else if (cmd == "spatialization-select-affected-notes") {
      
            SpatSymbol* spat = static_cast<SpatSymbol*>(e);
            int tickDuration = spat->tick() + spat->duration()*480;
            
            // We first mark the notes of the first chord.
            Segment *segment = static_cast<Segment*>(e->parent());
          if(segment->element(e->track())->type() == Element::CHORD) {
                  // The spat symbol is on notes.
                  // We select all the notes, which are on the same
                  Chord *chord = static_cast<Chord*>(segment->element(e->track()));
                  QList<Note*> notes = chord->notes();
                  if(notes.count()) {
                        // Select the first note using SELECT_SINGLE to start a new selection.
                        score()->select(notes.at(0),SELECT_SINGLE, e->staffIdx());
                        for(int i=1; i< notes.count();i++) {
                              score()->select(notes.at(i),SELECT_RANGE, e->staffIdx());
                        }
                  }
            }else if(segment->element(e->track())->type() == Element::REST) {
                  // The spat symbol is on a rest.
                  score()->select(segment->element(e->track()),SELECT_SINGLE, e->staffIdx());
            }
            
            // We now search the last segment, that is affected by the spat symbol.
            Segment *lastSegment = segment;
            while(segment && segment->tick() < tickDuration) {
                  // We only respect chords and rests as end.
                  Element *ele = segment->element(e->track());
                  if(ele->type() == Element::CHORD || ele->type() == Element::REST) {
                        lastSegment = segment;
                  }
                  // This moves to the next segment, even if it is on the next measure.
                  segment = segment->next1();
            }
            
            if(lastSegment->element(e->track())->type() == Element::CHORD) {
                  Chord *chord = static_cast<Chord*>(lastSegment->element(e->track()));
                  QList<Note*> notes = chord->notes();
                  if(notes.count()) {
            ...
            }else if(lastSegment->element(e->track())->type() == Element::REST) {
                  score()->select(lastSegment->element(e->track()),SELECT_RANGE, e->staffIdx());
            }
      }
